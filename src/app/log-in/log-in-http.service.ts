...
@Injectable({
 ...
 {
   return this.http.post('http://0f626498.ngrok.io/api/login',{email,password}) ;
 }
...
/**Sends a request to server with the variables that user entered to sign up a new user */
 ...
 {
   return this.http.post('http://0f626498.ngrok.io/api/signup',{email,password,password_confirmation,name,gender,birthday,country,city}) ;
 }
/**Loggs out the user from the website */
 logOut():Observable<any>{
    return this.http.delete('http://0f626498.ngrok.io/api/logout');
  }
  }
