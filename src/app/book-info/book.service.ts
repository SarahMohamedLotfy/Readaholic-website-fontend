...
/**constructor of book service */
  ...
  //  return this.http.get<book>('http://localhost:3000/book/'+id);
    ...
    return this.http.get('http://ec2-3-87-221-152.compute-1.amazonaws.com/api/books/show?book_id='+id);
 }

 /**gets the reviews of the book */
 getBookReviews(id: number): Observable<any> {
  //let params = new HttpParams().set("bookId",id);
  return this.http.get<any>('http://ec2-3-87-221-152.compute-1.amazonaws.com/api/showReviewsForABook?bookId='+id);
 ...
 getUserBookInfo(id: number): Observable<any> {
  return this.http.get<userBookInfo>('http://ec2-3-87-221-152.compute-1.amazonaws.com/api/showReviewForBookForUser?bookId='+id);
}
...
createReview(bookId:number, shelf: number, body: string, rating: number): Observable<any> {
  if(body == "")
  {
    return this.http.post('http://ec2-3-87-221-152.compute-1.amazonaws.com/api/reviwes/create',{bookId,shelf,rating});
  }
  else
  {
    return this.http.post('http://ec2-3-87-221-152.compute-1.amazonaws.com/api/reviwes/create',{bookId,shelf,body,rating});
  }
