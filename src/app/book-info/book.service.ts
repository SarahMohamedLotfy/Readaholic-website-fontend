...
/**constructor of book service */
  ...
  //  return this.http.get<book>('http://localhost:3000/book/'+id);
    ...
    return this.http.get('http://0f626498.ngrok.io/api/books/show?book_id='+id);
 }

 /**gets the reviews of the book */
 getBookReviews(id: number): Observable<any> {
  //let params = new HttpParams().set("bookId",id);
  return this.http.get<any>('http://0f626498.ngrok.io/api/showReviewsForABook?bookId='+id);
 ...
 getUserBookInfo(id: number): Observable<any> {
  return this.http.get<userBookInfo>('http://0f626498.ngrok.io/api/showReviewForBookForUser?bookId='+id);
}
...
createReview(bookId:number, shelf: number, body: string, rating: number): Observable<any> {
  if(body == "")
  {
    return this.http.post('http://0f626498.ngrok.io/api/reviwes/create',{bookId,shelf,rating});
  }
  else
  {
    return this.http.post('http://0f626498.ngrok.io/api/reviwes/create',{bookId,shelf,body,rating});
  }
